...
function mkFullPath(pathToCreate) {
  try {
    _fs["default"].mkdirSync(pathToCreate);
  } catch (e) {
    if (e.code === 'ENOENT') {
      // Create parent dir
      mkFullPath(_path["default"].dirname(pathToCreate)); // And now try again
...
function loadFingerprint(serverId, knownHostsPath, cb) {
  try {
    _fs["default"].accessSync(knownHostsPath);
  } catch (e) {
    return cb(null);
  }
  ...
  _readline["default"].createInterface({
    input: _fs["default"].createReadStream(knownHostsPath)
...
function storeFingerprint(serverId, knownHostsPath, fingerprint, cb) {
  ...
  _lockFingerprintFromAppending[serverId] = 1; // If file doesn't exist, create full path to it

  try {
    _fs["default"].accessSync(knownHostsPath);
  } catch (_) {
    mkFullPath(_path["default"].dirname(knownHostsPath));
  }

  _fs["default"].appendFile(knownHostsPath, serverId + ' ' + fingerprint + _os.EOL, 'utf8', function (err) {
...
var TrustStrategy = {
  ...
  TRUST_ON_FIRST_USE: function TRUST_ON_FIRST_USE(config, onSuccess, onFailure) {
    ...
    var socket = _tls["default"].connect(config.address.port(), config.address.resolvedHost(), tlsOpts, function () {
      var serverCert = socket.getPeerCertificate(
      /* raw= */
      true);
      ...
      var serverFingerprint = _crypto["default"].createHash('sha512').update(serverCert.raw).digest('hex');

      var knownHostsPath = config.knownHostsPath || _path["default"].join(userHome(), '.neo4j', 'known_hosts');

      var serverId = config.address.asHostPort();
